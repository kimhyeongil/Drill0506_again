...
def handle_events():
    ...
    for event in events:
        if event.type == SDL_QUIT:
            ...
            event.type == SDL_MOUSEBUTTONDOWN and event.button == SDL_BUTTON_LEFT
        ):  # 마우스 클릭이 있으면
...
def update_world():
    ...
    if target_exists:
        if t <= 1.0:
            ...
            set_new_target_arrow()
    elif points:  # 목표 지점이 없을 때, 새로운 손이 생기면
        set_new_target_arrow()
...
while running:
    ...
    update_world()  # 월드 내 객체들의 상호작용을 계산하고 update한다.
